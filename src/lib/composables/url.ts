// https://github.com/vueuse/vueuse/blob/f29b141b3578a0434ad6af11b05610827449d8ed/packages/core/useUrlSearchParams/index.ts
import {
  defaultWindow,
  pausableWatch,
  useEventListener,
...
export function useUrlSearchParams<T extends Record<string, any> = UrlParams>(
  ...
  function constructQuery(params: URLSearchParams) {
    ...
    const index = hash.indexOf("?");
    if (index > 0)
      return `${window.location.search || ""}${hash.slice(0, index)}${
        stringified ? `?${stringified}` : ""
      }`;
  ...
  const { pause, resume } = pausableWatch(
    ...
    () => {
      ...
      if (!writingFromOnChange) {
        write(params, false, true);
      }
      writingFromOnChange = false;
    },
    { deep: true }
  );
